...
use cdk_payment_processor_lexe::backend::LexeBackend;
use cdk_payment_processor_lexe::settings::Config;
use tokio::signal;
...
async fn main() -> Result<()> {
    ...
    let backend = Arc::new(LexeBackend::new(&cfg).await?);
    ...
    tracing::info!(
        "Starting Lexe payment processor on {}://{}:{}",
        scheme,
        cfg.address,
        cfg.port
    );
